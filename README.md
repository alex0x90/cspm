...
- **Template Method** — `BaseCheck.execute()` wraps every check with consistent error handling.
- **Registry** — `SERVICE_REGISTRY` in `detector.py` maps service names to check classes and boto3 clients.
- **Factory** — `aws_client.get_client()` creates and caches boto3 sessions/clients.
- **Class-Level Metadata** — Check metadata (`severity`, `description`, `remediation`, `impact`) is declared as class attributes, eliminating boilerplate. A shared `_make_finding()` helper auto-populates common fields.
